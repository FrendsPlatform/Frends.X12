...
{
    internal static string CreateX12FromXml(
        string xml, CancellationToken cancellationToken)
    {
        ...
        {
            var documentType = DetermineDocumentType(ediXml);
    ...
    {
        var ediMessages = ediXml.DescendantsAndSelf(documentType.Name);
    ...
    private static T DeserializePart<T>(XElement xElement)
    {
        ...
        return part ?? throw new FormatException($"Could not deserialize element {xElement.Name.LocalName}");
    }
}